    ...
    P: Provider + 'static + Clone,
{
    ...
    ///
    /// Without the feature there is no server to drive it, so `GRPC_ADDRESS` is ignored rather
    /// than stalling aggregation entirely.
    fn external_aggregation(&self) -> bool {
        cfg!(feature = "agglayer") && self.requester_config.grpc_addr.is_some()
    }
    ...
    pub async fn run(self: Arc<Self>) -> Result<()> {
        ...
        if let Some(addr) = self.requester_config.grpc_addr {
